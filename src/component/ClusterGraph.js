...
const ClusterGraph = ({ cluster_ids, setClusters }) => {
  ...
  const renderChart = () => {
    ...
    let rects = svg.selectAll('rect')
      ...
      .attr('y', d => yScale(d.y))
      .style('fill', 'none')
      .style('stroke', d => (click_cluster_ids.includes(d.cluster_id) || cluster_ids.includes(d.cluster_id)) ? 'yellow' : 'none')
      .style('stroke-width', d => (click_cluster_ids.includes(d.cluster_id) || cluster_ids.includes(d.cluster_id)) ? '6px' : '0px');
  ...
  return (
      ...
      <div className="chart-part">
         {showAlert && <Alert severity="info">It's enough! click play</Alert>}
