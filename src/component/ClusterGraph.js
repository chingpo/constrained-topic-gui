...
const hullPadding = 30; // You can adjust this value based on your requirements
...
const patch_width = 40
const patch_height = 40
...
const ClusterGraph = ({ cluster_ids, setClusters,data   }) => {
  ...
  const hullIds = round >= 4 ? Array.from({length: 20}, (_, i) => i) : [...new Set([...cluster_ids, hoveredClusterId])];
  return (
    <div>
      <Snackbar
        open={openSnackbar}
      ...
      {(zoom) => (
        ...
        <svg width={width} height={height} 
         ...
         onMouseLeave={() => {
          ...
          {data.map((d, i) => ( 
            <g key={i} transform={zoom.toString()}>
              ...
              {hullIds.map(cluster_id => {
                ...
                const points = clusterData.map(d => [x_scale(d.x)+patch_width/2, y_scale(d.y)+patch_height/2]);
          ...
          <img 
            style={{ width: '7rem', display: 'block', margin: 'auto', marginTop: '70px' }} 
