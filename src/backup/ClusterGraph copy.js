...
const ClusterGraph = ({cluster_ids,setClusters}) => {
  ...
  const addImage = (ctx,  xScale, yScale) => {
    data.forEach((d, i) => {
      const image = new Image();
      image.onload = function () {
        d.width = 20;
        d.height = 20;
        ctx.drawImage(image, xScale(d.x), yScale(d.y)+20, d.width, d.height);
        // 如果图像被选中，添加高亮背景
        if (click_cluster_ids.includes(d.cluster_id)||cluster_ids.includes(d.cluster_id)) {
            ctx.strokeStyle = 'rgba(255, 255, 0, 1)'; // 黄色
            ctx.lineWidth = 2; // 边框宽度
            ctx.strokeRect(xScale(d.x), yScale(d.y), d.width, d.height);
          }

        // paint center
        // center_json.forEach((d,i)=>{
        //     ctx.font = "30px Arial";
        //     ctx.fillText(`cluster_id: ${d.cluster_id}`,xScale(d.x),yScale(d.y));
        // })
      };
      image.onerror = function (err) {
        console.log("err", err);
      };
      image.src = home_url + d.img_name;
...
//     if (!click_cluster_ids.includes(i.cluster_id)) {
//         setClickClusterIds([
//           ...click_cluster_ids,
//           i.cluster_id
//         ]);
//       }
//       setClusters(click_cluster_ids);
//   }
const choose_topic = (d,i) =>{
  ...
  const renderChart = () => {
    ...
    // add image and centers in canvas
    addImage(ctx,xScale,yScale);
